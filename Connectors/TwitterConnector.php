...
class TwitterConnector
{
    ...
    private function php_self($dropqs=true)
    {
        ...
        $protocol = 'http';
        if (null !== $request->getServer('HTTPS') && strtolower($request->getServer('HTTPS')) == 'on') {
            $protocol = 'https';
        } elseif (null !== $request->getServer('SERVER_PORT') && ($request->getServer('SERVER_PORT') == '443')) {
            $protocol = 'https';
        }
    ...
    {
        $request = $this->di->get('request');
        $url = parse_url($request->getServer('REQUEST_URI'));
        $params = array();
    ...
    public function request_token()
    {
        ...
        if ($code != 200) {
            ...
            $this->di->get('logger')->error("There was an error communicating with Twitter. {$this->response['response']}");
        ...
        $session->set('twitterOauth', $this->extract_params($this->response['response']));

        // check the callback has been confirmed
        $twitterOauth = $session->get('twitterOauth');
        if ($twitterOauth['oauth_callback_confirmed'] !== 'true') {
            $this->di->get('logger')->begin();
